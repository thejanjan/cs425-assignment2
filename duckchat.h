...
#define SERVER_KEEPALIVE 120

#define TOPOLOGY_RENEW 60
...
/* Define codes for server-to-server types.  These are messages sent between servers. */
#define S2S_JOIN 8
#define S2S_LEAVE 9
#define S2S_SAY 10
...
// Cannot use packed attribute for these, since we're storing the full address
struct request_server_join {
        request_t req_type; /* = S2S_JOIN */
        sockaddr_in address;
        char req_channel[CHANNEL_MAX];
};

struct request_server_leave {
        request_t req_type; /* = S2S_LEAVE */
        sockaddr_in address;
        char req_channel[CHANNEL_MAX];
};

struct request_server_say {
